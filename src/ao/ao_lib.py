...
def get_hf_submodule(model, layer: int, use_lora: bool = False):
    """
    Get the residual stream submodule for the LLM at a specific layer.

    Handles three structures:
    - Plain HF transformer:                  model.model.layers[i]
    - PEFT-wrapped transformer:              model.base_model.model.model.layers[i]
    - Gemma 3 VLM (AutoModelForImageTextToText): model.model.language_model.layers[i]
    - PEFT-wrapped Gemma 3 VLM:              model.base_model.model.model.language_model.layers[i]
    """
    model_name = model.config._name_or_path
    is_gemma3_vlm = "gemma-3" in model_name and any(
        sz in model_name for sz in ("4b", "12b", "27b")
    )

    if use_lora:
        # PEFT-wrapped: navigate through base_model.model
        base = model.base_model.model
    else:
        base = model

    if is_gemma3_vlm:
        # Gemma 3 multimodal: LM layers are nested under language_model
        return base.model.language_model.layers[layer]

    if any(k in model_name for k in ("gemma", "mistral", "Llama", "Qwen")):
        return base.model.layers[layer]

    raise ValueError(f"Please add submodule access pattern for model {model_name}")
