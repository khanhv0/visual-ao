...
Activation Oracle inference library, lifted from Karvonen et al.'s public demo
notebook (activation_oracle_demo.ipynb) and adapted for Gemma 3 27B-IT.

Changes from the original:
- Added google/gemma-3-27b-it to LAYER_COUNTS (62 layers)
...
LAYER_COUNTS = {
    ...
    "google/gemma-3-27b-it": 62,   # ADDED for our target model
...
def get_hf_submodule(model, layer: int, use_lora: bool = False):
    """
    Gets the residual stream submodule at `layer` for HF transformers.

    Handles three model structures:
      1. Text-only LLM (e.g. Gemma-2-9B, Llama):
           model.model.layers[layer]
      2. VLM (Gemma3ForConditionalGeneration) — inner Gemma3Model has
         language_model:
           model.model.language_model.layers[layer]
      3. Any of the above wrapped by PEFT (PeftModel):
         PEFT adds one .base_model.model layer; we unwrap it first.

    The use_lora flag performs explicit PEFT unwrapping before navigation,
    which was the ONLY path that worked before — this fix makes both paths
    work by detecting the structure dynamically instead.
    """
    # Step 1: unwrap PEFT if present
    if use_lora and hasattr(model, "base_model"):
        base = model.base_model.model
    elif hasattr(model, "base_model"):
        # PEFT-wrapped but use_lora=False: still unwrap to get to the real model
        base = model.base_model.model
    else:
        base = model

    # Step 2: navigate to the transformer layers
    # base is now the top-level model class (e.g. Gemma3ForConditionalGeneration,
    # Gemma2ForCausalLM, LlamaForCausalLM, etc.)

    # Case A: VLM — Gemma3ForConditionalGeneration
    # Structure: base.model (Gemma3Model) → .language_model (Gemma3TextModel) → .layers
    if hasattr(base, "model") and hasattr(base.model, "language_model"):
        return base.model.language_model.layers[layer]

    # Case B: Text-only LLM — standard HF causal LM
    # Structure: base.model → .layers
    if hasattr(base, "model") and hasattr(base.model, "layers"):
        return base.model.layers[layer]

    # Case C: bare transformer (no outer wrapper, e.g. Gemma3TextModel directly)
    if hasattr(base, "layers"):
        return base.layers[layer]

    raise ValueError(
        f"Cannot locate transformer layers in {type(base).__name__}. "
        f"Inspect with print(model) and add a branch. "
        f"Attributes: {[n for n, _ in base.named_children()]}"
    )
...
def get_hf_activation_steering_hook(
    ...
    """HF hook for activation steering — replaces residual stream at given positions."""
...
class TrainingDataPoint(BaseModel):
    ...
    feature_idx: int
    target_output: str
    target_input_ids: Optional[list]
    target_positions: Optional[list]
    ds_label: Optional[str]
    meta_info: Mapping[str, Any] = {}
...
@dataclass
class OracleResults:
    oracle_lora_path: Optional[str]
    target_lora_path: Optional[str]
    target_prompt: str
    act_key: str
    oracle_prompt: str
    ground_truth: str
    ...
    full_sequence_responses: list
    segment_responses: list
...
def create_training_datapoint(
    ...
    if isinstance(full_prompt_ids, BatchEncoding):
        full_prompt_ids = full_prompt_ids["input_ids"]

    if isinstance(input_prompt_ids, torch.Tensor):
        input_prompt_ids = input_prompt_ids.tolist()

    if isinstance(full_prompt_ids, torch.Tensor):
        full_prompt_ids = full_prompt_ids.tolist()
    ...
    if (
        isinstance(full_prompt_ids, list)
        and full_prompt_ids
        and isinstance(full_prompt_ids[0], list)
    ):
        full_prompt_ids = full_prompt_ids[0]
    ...
    # print("=" * 80)
    # print("input_prompt_ids type:", type(input_prompt_ids))
    # print("full_prompt_ids type:", type(full_prompt_ids))
    # print("ids_list type:", type(ids_list))
    # print("first token type:", type(ids_list[0]))
    # print("first 20 ids:", ids_list[:20])
    ...
    # try:
    #     print(tokenizer.decode(ids_list[:300]))
    # except Exception as e:
...
def _create_oracle_inputs(
    ...
    if "segment" in oracle_input_types:
        # TODO: check hypothesis that directive is encoded at the turn bondary -> concentrated around that point
...
print("ao_lib loaded successfully")